...
@Entity
@Table(name = "wearable_item", schema = "public", catalog = "charactermanager")
class WearableItemEntity {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val that = other as WearableItemEntity
        return id == that.id &&
                name == that.name &&
                level == that.level &&
    ...
    override fun hashCode(): Int {
        return Objects.hash(id, name, level, actions, price, hands, bulk, contains, usage, amount, action0, action0Desc, action1, action1Desc, description, src, url)
    }
}